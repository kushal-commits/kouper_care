-- Populate comprehensive sample data using existing profiles
DO $$
DECLARE
    ...
    counter INT := 0;
BEGIN
    -- Get the first available profile ID (should exist from previous migrations)
    SELECT id INTO first_profile_id FROM public.profiles LIMIT 1;
    
    -- If no profiles exist, skip the data population
    ...
    LOOP
        ...
        -- Create clinician notes (one at a time)
