...
SELECT 
    ...
    CASE 
        ...
        ELSE 'cancelled'
    END as status,
    NULL::uuid as primary_nurse_id, -- Will be assigned when auth is implemented
    NULL::uuid as coordinator_id,   -- Will be assigned when auth is implemented
    CASE 
...
SELECT DISTINCT
    ...
    END as notes,
    NULL::uuid as assessor_id -- Will be assigned when auth is implemented
FROM public.patients p
