-- Populate comprehensive sample data for all patients
DO $$
DECLARE
...
BEGIN
    ...
    ot_id := COALESCE(profile_ids[2], nurse_id);
    pt_id := COALESCE(profile_ids[3], nurse_id);
    cm_id := COALESCE(profile_ids[4], nurse_id);
    ...
    LOOP
        ...
        (
            'Progress Note - ' || patient_record.first_name,
            'Patient showing ' ||
            CASE WHEN counter % 4 = 0 THEN 'steady improvement in ADL performance. Family reports increased confidence in home safety.'
                 WHEN counter % 4 = 1 THEN 'moderate progress with some setbacks. Continue current intervention plan.'
                 WHEN counter % 4 = 2 THEN 'excellent progress. Patient exceeding expected outcomes for this stage.'
                 ELSE 'slower than expected progress. Recommend increased therapy frequency.' END,
            'note',
            CASE WHEN counter % 2 = 0 THEN ot_id ELSE pt_id END,
            patient_record.id,
            ('{"note_date": "' || (CURRENT_DATE - INTERVAL '2 days')::text || '"}')::jsonb
        ) RETURNING id INTO source_doc_id;
        
        -- Create data source links
        ...
        (
            ...
            assessment_id,
            source_doc_id,
            'Bathing requires moderate assistance due to balance concerns. Dressing upper body shows good independence.',
            0.92 + (counter * 0.01)
        );
