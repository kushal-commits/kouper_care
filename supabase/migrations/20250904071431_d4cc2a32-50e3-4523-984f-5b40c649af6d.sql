...
SELECT 
    ...
    CASE 
        ...
        ELSE 'cancelled'
    END as status,
    NULL::uuid as primary_nurse_id,
    NULL::uuid as coordinator_id,
    CASE 
...
SELECT DISTINCT
    p.id as patient_id,
    CASE s.series % 5
        WHEN 0 THEN 'start_of_care'
        WHEN 1 THEN 'follow_up'
        WHEN 2 THEN 'transfer'
        WHEN 3 THEN 'resumption'
        ELSE 'discharge'
    END as assessment_type,
    (p.created_at::date + (s.series * 14)) as assessment_date,
    -- Generate varied but realistic ADL scores (0-5 scale)
    (2 + (abs(hashtext(p.id::text || s.series::text || 'bathing')) % 4)) as bathing_score,
    (2 + (abs(hashtext(p.id::text || s.series::text || 'dressing_upper')) % 4)) as dressing_upper_score,
    (2 + (abs(hashtext(p.id::text || s.series::text || 'dressing_lower')) % 4)) as dressing_lower_score,
    (2 + (abs(hashtext(p.id::text || s.series::text || 'grooming')) % 4)) as grooming_score,
    (3 + (abs(hashtext(p.id::text || s.series::text || 'eating')) % 3)) as eating_score,
    (2 + (abs(hashtext(p.id::text || s.series::text || 'toileting')) % 4)) as toileting_score,
    (1 + (abs(hashtext(p.id::text || s.series::text || 'transferring')) % 5)) as transferring_score,
    (1 + (abs(hashtext(p.id::text || s.series::text || 'ambulation')) % 5)) as ambulation_score,
    (2 + (abs(hashtext(p.id::text || s.series::text || 'medication')) % 4)) as medication_mgmt_score,
    CASE s.series % 4
