...
(gen_random_uuid(), 'coordinator1@example.com', 'Dr. James Wilson', 'case_manager'),
...
SET 
  ...
  coordinator_id = (SELECT id FROM profiles WHERE role = 'case_manager' LIMIT 1)
WHERE patient_id = '061a562c-e34d-4c2b-b36d-3d02465de7b2';

-- Update assessments to have proper assessor assignments  
UPDATE assessments 
SET assessor_id = (SELECT id FROM profiles WHERE role IN ('nurse', 'case_manager') ORDER BY random() LIMIT 1)
